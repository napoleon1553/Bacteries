...
engine = create_engine("postgresql+psycopg2://postgres:Batik007555.@localhost/postgres")
Session = sessionmaker(bint=engine)
...
def find(vector: str):
    ...
    for num, sign in enumerate(vector):
        if sign == '<':
            first = num
        elif sign == '>' and first is not None:
            second = num
...
for id in list(players):
    ...
    s.merge(player)  # Обновляем или вставляем игрока в базу данных
